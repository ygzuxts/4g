...
// CRC24Q
const uint32_t CRC24Q_TABLE[256] = {
...
// 计算 CRC24Q
uint32_t calculate_crc24q(const uint8_t *data, size_t length)
{
    uint32_t crc = 0;
...
void process_rtcm_data(uint8_t *buffer, uint16_t length)
{
    if (length < 6)
    {
        printf("data length isnot enough\n");
        return;
    }
    ...
    for (uint16_t i = 0; i < length - 6; i++)
    {
        ...
        if (buffer[i] == 0xD3)
        {
            uint16_t payload_length = ((buffer[i + 1] & 0x03) << 8) | buffer[i + 2]; // 获取负载长度
            uint16_t total_length = payload_length + 6;                              // 包含头部、长度和校验的总包长

            if (i + total_length > length)
            {
                printf("wait for more date\n");
                break;
            }

            // 提取负载数据
            // 提取crc24Q校验值
            uint32_t received_crc = (buffer[i + 3 + payload_length] << 16) |
                                    (buffer[i + 4 + payload_length] << 8) |
                                    buffer[i + 5 + payload_length];

            // 计算 CRC 校验
            uint32_t calculated_crc = calculate_crc24q(&buffer[i], payload_length + 3);
